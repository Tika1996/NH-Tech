...
export default function PieceDetailPage() {
  ...
  const mainImg = formatImageUrl(piece?.image) || 'https://images.unsplash.com/photo-1591488320449-011701bb6704?w=600';
  // Use real gallery images if uploaded from app, otherwise fallback
  const imagesGallery = Array.from(new Set([
    mainImg,
    ...(piece?.galleryImages || []).map(img => formatImageUrl(img))
  ])).filter(Boolean);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          <div style={{ background: 'var(--bg-card, #fff)', borderRadius: '24px', padding: '24px', border: '1px solid var(--border-color, #e2e8f0)', boxShadow: '0 8px 30px rgba(0,0,0,0.04)' }}>
            ...
            {piece.ref && (
              <div style={{ fontSize: '0.88rem', color: 'var(--text-secondary)', marginBottom: '12px' }}>
                Référence Officielle : <strong style={{ color: 'var(--text-primary)' }}>{piece.ref}</strong>
              </div>
            )}

            <p className="nh-desc-block">
          ...
          <div style={{ background: 'var(--bg-card, #fff)', borderRadius: '24px', padding: '20px 24px', border: '1px solid var(--border-color, #e2e8f0)', boxShadow: '0 8px 30px rgba(0,0,0,0.04)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              <span className="nh-spec-badge">⚡ Pièce détachée testée</span>
              <span className="nh-spec-badge">🛡️ Emballage anti-statique sécurisé</span>
              <span className="nh-spec-badge">📄 Facture et garantie</span>
            </div>
          </div>
        </div>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                {piece.brand && <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 700 }}>• {piece.brand}</span>}
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px', background: 'var(--bg-card, #fff)', padding: '24px', borderRadius: '24px', border: '1px solid var(--border-color, #e2e8f0)', boxShadow: '0 8px 30px rgba(0,0,0,0.03)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)' }}>7j/7 - Support technique</div>
          </div>
        </div>
      </div>
    </div>
